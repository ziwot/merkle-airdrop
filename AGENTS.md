...
- **cwd rules:** `infra/scripts/*` must run with cwd = `infra/` (`TESTDATA_PATH = "./testdata"`, and `makeToken` runs `ligo` with `cwd: "../contract"`) → use `npm --prefix infra run …` or the make targets. `contract/src/generate_metadata_bytes.sh` is the opposite: it hardcodes `contract/src/metadata.json`, so it only works from the repo root.
