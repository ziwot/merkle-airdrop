...
if (Configure::read('debug')) {
    ...
    $this->prepend('css', $this->Html->css(['bootstrap.min']));
}
$this->prepend(
    'css',
    $this->Html->css(['font/bootstrap-icons', 'font/bootstrap-icon-sizes']),
);
...
if (Configure::read('debug')) {
    $this->prepend(
        'script',
        $this->Html->script(['htmx.min', 'popper', 'bootstrap']),
    );
} else {
    $this->prepend(
        'script',
        $this->Html->script(['htmx.min', 'popper.min', 'bootstrap.min']),
    );
}
$this->prepend('script', $this->Html->script('alpine.min', ['defer' => true]));
?>
<!doctype html>
<?= $this->fetch('html') ?>
    <head>
