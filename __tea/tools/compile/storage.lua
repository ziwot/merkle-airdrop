...
for id, vars in pairs(_computed.DEPLOYS) do
	...
	if not _ok then
		log_warn(
			"Failed to read metadata from 'build/metadata.json'! ('metadata' argument wont be available during initial storage generation)"
		)
	end
	_metadata = "0x" .. string.tohex(_metadata) -- get hex

	-- override token-addr
	local _sandboxTokenFile = fs.read_file("deploy/sandbox-token.json", { binaryMode = true })
	local _ok, _parsed = hjson.safe_parse(_sandboxTokenFile)
	ami_assert(_ok, "Invalid sandbox-token.json!")
	local _tokenAddress = _parsed.contractAddress
	vars["token-addr"] = string.interpolate('"${ADDR}"', { ADDR = _tokenAddress })

	vars = util.merge_tables(_computed.LIGO_VARS, vars, true)
	vars = util.merge_tables(vars, {
		DEPLOY = id,
		metadata = _ok and _metadata or nil, --[[requires 2 pass]]
	}, true)
