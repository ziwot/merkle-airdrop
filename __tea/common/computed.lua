...
for _, v in pairs(_deploys) do
	v.INITIAL_STORAGE_ARGS = am.app.get_configuration(
		{ "ligo", "initial-storage-args" },
...
return {
	...
	SANDBOX_VARS = {
		ENGINE = _containerEngine,
		NAME = am.app.get_configuration({ "sandbox", "name" }, "sandbox-" .. am.app.get("id", "tezos")),
	...
	METADATA_VARS = {
		...
		OFFCHAIN_VIEW_EXP_SUFFIX = am.app.get_configuration(
			{ "metadata", "offchain-view-expression-suffix" },
			"_off_chain_view"
		),
	},
}
